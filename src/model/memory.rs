...
impl<'a> Memory<'a> {
    pub fn load_elf(path: &str) -> Self {
        ...
        let elf_file =
            ElfBytes::<'a>::minimal_parse(file_data_static).expect("Failed to parse ELF file");
    ...
    pub fn get_elf_text(&self) -> &'a [u8] {
        let text_sh = self
            .sh_map
            .get(".text")
            .expect("Failed to get .text section");

        self.bytes
            .section_data(text_sh)
            .expect("failed to get elf text section")
            .0
    }
