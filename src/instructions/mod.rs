...
#[inline(always)]
pub fn bit_range_inclusive(i: u32, low: u32, high: u32) -> u32 {
...
mod tests {
    ...
    fn test_get_bit_range() {
        let instr = 0xb084;
        let range = bit_range_inclusive(instr, 12, 15);
        assert_eq!(bit_range_inclusive(instr, 12, 15), 0xb);
        assert_eq!(bit_range_inclusive(instr, 8, 11), 0);
        assert_eq!(bit_range_inclusive(instr, 4, 7), 8);
        assert_eq!(bit_range_inclusive(instr, 0, 3), 4);
    }
}
